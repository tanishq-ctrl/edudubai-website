...
export function PathwayTable() {
  return (
    <section className="border-t border-line bg-surface-sunken py-section-sm">
      <Container>
        ...
        <div className="mt-10 overflow-hidden rounded-sm border border-line bg-surface-raised">
            ...
            <table className="w-full min-w-[42rem] border-collapse text-left">
              ...
              <tbody>
                {pathways.map((p) => (
                  <tr
                    key={p.code}
                    className="border-b border-line align-middle transition-colors duration-fast last:border-0 hover:bg-crimson-50"
                  >
                    <th
                      scope="row"
                      className="py-4 pl-6 pr-6 text-[15px] font-medium text-content-strong"
                    >
                      {p.role}
                    </th>
                    <td className="py-4 pr-6 text-[15px] leading-relaxed text-content-muted">
                      {p.rationale}
                    </td>
                    <td className="py-4 pr-6">
                      <Link
                        href={`/courses/${p.slug}`}
                        /* A programme code is a credential, so this is one of
                           the places gold is still the right accent. */
                        className="inline-flex items-center rounded-full border border-gold-400 px-3.5 py-1.5 text-xs font-semibold text-gold-ink transition-colors duration-fast hover:bg-gold-400 hover:text-ink-950"
                      >
                        {p.code}
                      </Link>
                    </td>
