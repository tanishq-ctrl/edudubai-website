 ...
 * need a light disc to sit on and a dark band behind that -- on the gold band
 * this originally sat on, gold-on-gold turned to mud.
 */
const seals = [
      ...
      <div className="flex flex-col gap-12 lg:flex-row lg:items-center lg:justify-between lg:gap-20">
        ...
        <Stagger className="flex flex-wrap items-center gap-6" step={90} variant="scale">
          {seals.map((seal) => (
            <div
              key={seal.code}
              className="relative h-[8.5rem] w-[8.5rem] rounded-full bg-white p-3.5 shadow-xl"
            >
              <Image
                src={seal.src}
                alt={seal.alt}
